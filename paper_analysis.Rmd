...
dat_sum <- dat_filter %>%
  filter(type == "VOC" | type == "negative" | type == "NR-4023 anti-RSV pooled serum – low") %>%
...
tab_2_df = read_excel("paper_audit.xlsx", sheet = "Validation; RSV B") %>%
  ...
  group_by(`sample ID`) %>%
  summarise(count = n(), mean = round(mean(ND50 * 0.94), 0), geo_mean = geo_mean(ND50), mean_IU = geo_mean * 0.94, mean_IU_rounded = round(mean_IU, 0)) %>%
  mutate(reference = round(case_when(
...
tab_5_b150 = b_df %>%
  ...
  summarise(
    n = n(),
    geo_mean = round(geo_mean(ND50 * 0.94), 0),
    geo_mean_unrounded_cf = round(geo_mean(ND50 * 0.94), 0),
    mean = round(mean(ND50 * 0.94), 0),
    mean_unrounded_cf = round(mean(ND50 * 0.94), 0),
    virus = virus) %>%
  distinct() %>%
  ungroup() %>%
  flextable() %>%
